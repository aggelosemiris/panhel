...
import { normalizeQuizText, getTheoryQuizQuestions } from '../config/theoryQuiz.ts';
import { useQuiz } from '../context/QuizContext.tsx';
import { updateStudentStats } from '../services/studentStats.ts';

type QuizStatus = 'correct' | 'wrong' | null;

export default function TheoryQuizAttemptPage() {
  ...
  const chapterHasQuiz = Boolean(chapterID && getTheoryQuizQuestions(chapterID).length > 0);
  ...
  useEffect(() => {
    if (isComplete && isActiveQuizSession && subjectID && chapterID) {
      navigate(`/theory-quiz/${subjectID}/${chapterID}/result`, { replace: true });
    }
    ...
    return (
      <div className="simple-module-page">
        ...
        <div className="generator-panel theory-quiz-result-card">
          <h2>Το quiz δεν ξεκίνησε σωστά.</h2>
          <p>Επίλεξε ξανά το κεφάλαιο για να δημιουργηθεί νέο quiz session με όλες τις ερωτήσεις.</p>
  ...
  const handleVerify = async () => {
    ...
    try {
      await updateStudentStats({
        ...
        questionId: activeQuestion.id ?? `${chapter.id}-q${currentQuestionIndex + 1}`,
        isCorrect,
      });
    } catch {
      // The quiz should still progress even if the stats request fails.
    }
  ...
  const handleContinue = () => {
    ...
    if (!hasNextQuestion) {
      navigate(`/theory-quiz/${subjectID}/${chapterID}/result`);
    }
  };
